...
Airflow 자원점유의 특징은 DAG이나 task의 총량이 아니라 `동시에 동작 중인 태스크 수`에 선형적으로 비례한다는 점이다. 이로 인해 발생하는 가장 심각한 문제는 **메모리가 부족한 경우 airflow 프로세스가 사라진다**는 점이다. Airflow.cfg에서 동시에 실행가능한 task의 수를 적절히 제한해야하는데 이게 말이 쉽지 몇번 다운되보지 않고는 적정치를 알기가 힘들다.
