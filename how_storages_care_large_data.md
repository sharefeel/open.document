...
위 그림에서 볼때 SSD는 HDD의 6.5배 가격이다. 만약 HDFS를 SSD로 구성했을 경우 HDD 대비 두배의 속도가 나온다고 하면 그렇게 할 것인가? 그럴 수도 있고 아닐 수도 있겠지만 대체로 그렇지 않다.
1. 비용
   1. 기본적으로 SSD는 비싸다.
   2. 그리고 HDD대비 용량이 낮기 때문에 같은 용량을 위해서는 HDD보다 더 많은 서버수가 필요하다. 그만큼 서버구매비용, 상면비용, 운영비용이 추가투입되어야 한다. 
2. 리소스간 밸런스
