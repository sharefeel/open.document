...
<figure>
  <img src=".resources/how_storages_care_large_data/ssd_hdd_seq_write.png" width="600" title="Sequential Write Speed"/>
</figure>

<figure>
  <img src=".resources/how_storages_care_large_data/ssd_hdd_random_write.png" width="600" title="Random Write Speed"/>
</figure>
...
분산환경에서 CAP (Consistency, Availibility, Partition Tolerance)를 모두 만족하는 것은 불가능하다. 현생하는 저장소들은 다음 세가지 중 두가지 속성을 만족시키면서 각자의 길을 가고 있다.
...
* 공간의 절약 (수평확장이 불가능한 rdbms에서 공간은 매우 비싼 리소스이다)

반면 nosql 계열은 성능을 위해 의도적으로 정규화를 하지 않는 경우가 많다.
...
사실상 이 글을 쓴 목적은 기존의 RDBMS와 비교하여 현재 많이 사용되는 저장소들의 특징을 알아보자는 것이다. 두괄식이 더 좋은 구성인데 어김없이 하고 싶은 얘기가 뒤에 등장한다. 난 아무리 지식이 쌓이고 전문가가 되도 책은 못 쓸것 같다.
...
<figure>
  <img src=".resources/how_storages_care_large_data/kafka-topic-partition-layout.png" width="600" title="Kafka Topic Partition Layout">
</figure>
...
* 토폴로지
  ...
  * 메타 관리
    ...
    * 클러스터가 동기화 되지 않은 상태로 남아 버리는 현상이 발생하기도 한다
* IO 최적화
  * 데이터 저장 구조
    * `MemTable / SSTable / CommitLog` Write 요청이 오면 메모리의 MemTable에 업데이트한 후 CommitLog에 write 사실을 기록하고 성공했음을 클라이언트로 리턴. 주기적으로 디스크의 SSTable로 flush
      * `SSD for commitlog` Write ahead log 성격인 commit log는 저장에 필요한 IOPS가 매우 높기 때문에 별도의 SSD에 저장하기도 함
  * 쿼리별 동작
  ...
  * Compaction
    ...
    * Compaction을 실행하면 tomestone row를 제외한 후 sstable 파일을 다시 생성함
